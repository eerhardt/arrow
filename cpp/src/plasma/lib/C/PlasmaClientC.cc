...
int32_t org_apache_arrow_plasma_PlasmaClientC_connect(
     const char* store_socket_name, plasma::PlasmaClient** client)) {

  plasma::PlasmaClient* newClient = new plasma::PlasmaClient();
  Status status = newClient->Connect(store_socket_name, "");

  if (status.ok() && client) {
    *client = newClient;
  }
  return reinterpret_cast<int32_t>(status.code());
}
